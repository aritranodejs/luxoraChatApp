...
export async function getResponse(endpoint, method, body, headers, signal) {
    try {
        ...
        if (!API_URL) {
            console.error("⭐️ API_URL is not defined. Check your .env file");
        ...
        if (error instanceof TypeError && error.message.includes('fetch')) {
            console.error("⭐️ Server may be down or unreachable. Check server status and network connection.");
        }
